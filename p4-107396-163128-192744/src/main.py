...
def loadSamples():
    ...
    for i,filePath in enumerate(filePaths):
        ...
        samples.append(dict(id=i+1, name=name, path=filePath, label=label, index=index))
    return samples
...
if __name__ == '__main__':
    ...
    resultListSize = 3
    querySampleNames = ['beach_2', 'boat_5', 'cherry_3', 'pond_2', 'stHelens_2', 'sunset1_2', 'sunset2_2']

    samples = loadSamples()

    samplesDescriptors = descriptor.extractDescriptors(samples, descriptorsCacheDir, numSegments)
    for querySampleName in querySampleNames:
        querySample = getSample(samples, querySampleName)
        ...
        for distanceFunction in distanceFunctions:
            ...
            print('results for query '+querySampleName+' with distance '+ distanceFunction.__name__ +': ' + str(searchResult))
